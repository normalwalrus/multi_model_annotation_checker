...
# ── Config ────────────────────────────────────────────────────────────────────
LANGUAGE                = "ms"
PATH_TO_MANIFEST_FOLDER = "data/MALT_ST_ANNOTATED_RESULT/ms"
PATH_TO_AUDIO_FOLDER    = "data/to_pass_to_st/passed_30h_st_110326/MS"
OUTPUT_FOLDER           = "outputs"
 
MODEL = WhisperEvaluator(
    language=LANGUAGE,
...
def process_manifest(manifest_path: Path, audio_folder: str, output_folder: str) -> None:
    ...
    for idx, segment in enumerate(manifest.get("annotations", [])):
        ...
        try:
            whisper_text = MODEL.infer(audio_slice)
            # infer() may return a dict or a plain string – handle both
